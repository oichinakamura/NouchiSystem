...
Public Class C相続未登記実態調査
    ...
    Public Sub New()
        MyBase.New(True, True, "相続未登記農地等に係る実態調査", "相続未登記農地等に係る実態調査")
    ...
    Private Sub mvarBtnSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles mvarBtnSearch.Click
        ...
        pTBL農地 = SysAD.DB(sLRDB).GetTableBySqlSelect("SELECT V_農地.ID, V_農地.土地所在, V_農地.登記簿面積, V_農地.実面積, V_農地.所有者ID, V_農地.利用状況調査農地法, V_農地.利用状況調査荒廃 FROM(V_農地) WHERE (((V_農地.ID) Is Not Null) AND ((V_農地.所有者ID) Is Not Null) AND ((V_農地.田面積)>0) AND ((V_農地.大字ID)<>-1) AND ((V_農地.農地状況)<=19)) OR (((V_農地.ID) Is Not Null) AND ((V_農地.所有者ID) Is Not Null) AND ((V_農地.畑面積)>0) AND ((V_農地.大字ID)<>-1) AND ((V_農地.農地状況)<=19)) OR (((V_農地.ID) Is Not Null) AND ((V_農地.所有者ID) Is Not Null) AND ((V_農地.樹園地)>0) AND ((V_農地.大字ID)<>-1) AND ((V_農地.農地状況)<=19)) OR (((V_農地.ID) Is Not Null) AND ((V_農地.所有者ID) Is Not Null) AND ((V_農地.採草放牧面積)>0) AND ((V_農地.大字ID)<>-1) AND ((V_農地.農地状況)<=19)) ORDER BY V_農地.所有者ID;")
        ...
        pTBL住記情報 = SysAD.DB(sLRDB).GetTableBySqlSelect("SELECT M_住民情報.ID, M_住民情報.世帯No, M_住民情報.氏名, M_住民情報.住所, M_住民情報.住民区分, M_住民情報.生年月日 FROM(M_住民情報) WHERE (((M_住民情報.ID) Is Not Null));")
        ...
        mvarGrid明細.SetDataView(pTBL対象農家, "", "[ID]")
    ...
    Private Sub CreateTBL対象農家()
        pTBL対象農家 = New DataTable
        With pTBL対象農家
            .Columns.Add("ID", GetType(Decimal))
            .Columns.Add("世帯ID", GetType(Decimal))
    ...
    Private Sub Set対象農家テーブル(ByRef pRow As DataRow)
        Dim pFRow住記情報 As DataRow = pTBL住記情報.Rows.Find(pRow.Item("所有者ID"))
        Dim pFRow対象農家 As DataRow = pTBL対象農家.Rows.Find(pRow.Item("所有者ID"))

        If pFRow住記情報 IsNot Nothing AndAlso pFRow対象農家 IsNot Nothing Then
            With pFRow対象農家
                .Item("筆数計") += 1
            ...
            Allocation要因(pFRow対象農家, pRow)
        ElseIf pFRow住記情報 IsNot Nothing Then
            Dim pAddRow As DataRow = pTBL対象農家.NewRow

            With pAddRow
                .Item("ID") = pFRow住記情報.Item("ID")
                .Item("世帯ID") = pFRow住記情報.Item("世帯No")
                .Item("氏名") = pFRow住記情報.Item("氏名")
                .Item("住所") = pFRow住記情報.Item("住所")
                .Item("住民区分") = pFRow住記情報.Item("住民区分名")
                .Item("筆数計") = 1
                .Item("面積計") = Val(pRow.Item("登記簿面積").ToString)
                .Item("うち遊休農地筆数計") = IIf(Check遊休(pRow), 1, 0)
                .Item("うち遊休農地面積計") = IIf(Check遊休(pRow), Val(pRow.Item("登記簿面積").ToString), 0)
            ...
            pTBL対象農家.Rows.Add(pAddRow)

            Allocation要因(pAddRow, pRow)
        ElseIf pFRow住記情報 Is Nothing Then
            Dim pFindRow As DataRow = pTBL対象農家.Rows.Find(9999999999)
