...
Public Class CInterface18条解約
    ...
    Public Overrides Sub SetInterface(ByVal pPanel As HimTools2012.controls.FlowLayoutPanelPlus)
        ...
        set譲渡人(pPanel, "譲渡", False, False, False)
        ...
        With CType(Panel.AddCtrl(New HimTools2012.controls.GroupBoxPlus("申請情報", Me), "", em改行.改行あり), HimTools2012.controls.GroupBoxPlus)
            ...
            .Panel.AddCtrl(New ComboList(ListResource.S_Data, "解約の理由", mvarTarget, "申請理由A", "Text", ComboBoxStyle.DropDown), "解約の理由", em改行.改行あり)
            .Panel.AddCtrl(New DateTimePickerPlus(emRO.IsCanEdit, mvarTarget, "進達年月日"), "成立年月日", em改行.改行あり)
            .Panel.AddCtrl(New DateTimePickerPlus(emRO.IsCanEdit, mvarTarget, "解約年月日"), "解約年月日", em改行.改行あり)
            .Panel.AddCtrl(New DateTimePickerPlus(emRO.IsCanEdit, mvarTarget, "完了報告年月日"), "引渡年月日", em改行.改行あり)
